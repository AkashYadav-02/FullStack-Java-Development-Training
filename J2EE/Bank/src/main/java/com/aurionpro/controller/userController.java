...
public class userController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		case "back":
		 ...
		 case "logout":
			  logoutTransaction(request,response);
			  break;
			}
	...
	private void logoutTransaction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	...
	private void makeTransaction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Transaction ts=new Transaction( userId,  userAccount, 
				  userAccountType,  userBalance,  transactionType,
					 BAccount,  bamount,  bname);
		  ...
		  HttpSession session = request.getSession();
		       session.setAttribute("transaction", ts);
		       
		       System.out.println(ts.getTransactionType());
		   if( ts.getTransactionType().equals("deposite")) {
			   		System.out.println("If called");
			        makeDeposite(request,response);
			        loadPassbook(request,response);
...
//		   makeWithdraw(request, response);
	...
	private void makeWithdraw(HttpServletRequest request, HttpServletResponse response) {
		// TODO Auto-generated method stub
		
		HttpSession session = request.getSession();
		  Transaction ts = (Transaction) session.getAttribute("transaction");
		  System.out.println(" from inside make withdraw"+ts);
		   dbUtil.makeWithdraw(ts);
		 	}
	private void makeDeposite(HttpServletRequest request, HttpServletResponse response) {
			HttpSession session = request.getSession();
		  Transaction ts = (Transaction) session.getAttribute("transaction");
	  	  System.out.println(" from inside make withdraw"+ts);
		   dbUtil.makeDeposite(ts);
	}
